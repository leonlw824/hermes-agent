...
def test_display_toolset_name_strips_legacy_suffix():
    assert banner._display_toolset_name("yuanbao_tools") == "yuanbao"
...
def test_build_welcome_banner_uses_normalized_toolset_names():
    ...
    with (
        patch.object(
            ...
            return_value=(
                ["web"],
                [
                    {"name": "yuanbao", "tools": ["yb_query_group_info"]},
    ...
    assert "yuanbao:" in output
    assert "honcho:" in output
    assert "web:" in output
    assert "yuanbao_tools:" not in output
